	...
	for (size_t inst = 0; inst<instrsets.size(); inst++)
	{
		...
		for (size_t i=0; i<nInstrs; i++)
		{
			...
			for (uint32_t j=0; j<nRgns; j++)
			{
				...
				{
					wstringstream log;
					log << L"Sample " << realSampNum << "does not exist.";
					pRoot->AddLogItem(new LogItem(log.str().c_str(), LOG_LEVEL_ERR, L"VGMColl"));
				...
				//if (rgn->release_time < .001)
				//	rgn->release_time = .001;
				//if (rgn->decay_time < .001)
				//	rgn->decay_time = .001;
...
SynthFile* VGMColl::CreateSynthFile()
{
	...
	//SynthFile& synthfile = *pSynthFile;
	...
	for (size_t inst = 0; inst<instrsets.size(); inst++)
	{
		...
		for (size_t i=0; i<nInstrs; i++)
		{
			...
			for (uint32_t j=0; j<nRgns; j++)
			{
				...
				{
					wstringstream log;
					log << L"Sample " << realSampNum << L" does not exist.";
					pRoot->AddLogItem(new LogItem(log.str().c_str(), LOG_LEVEL_ERR, L"VGMColl"));
					realSampNum = finalSamps.size() - 1;
				}
